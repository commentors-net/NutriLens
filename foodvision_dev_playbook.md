...
- **Endpoints Working:** 
  ...
  - ?? `/meals/range` - Requires Firestore composite index (link provided below)

**?? FINAL SETUP STEP - Firestore Composite Index:**
...
- ??? **Server-side token validation** using Google's public keys
...
## ??? UNIFIED MULTI-SYSTEM ARCHITECTURE (2026-03-08)
...
**Goal:** Replace deterministic-only analysis with production-capable AI analysis plus feedback tuning.

#### 4.1 AI Service Selection
- [x] Integrate Google Gemini via `google.genai`
- [x] Keep deterministic fallback for reliability
- [x] Support model fallback chain for runtime resilience

#### 4.2 Food Detection
- [x] Call Gemini multimodal API for meal analysis
- [x] Post-process AI response into stable schema payload
- [x] Map returned labels to nutrition DB with safe unmatched handling
- [ ] Improve unknown-food handling quality with better normalization

#### 4.3 Portion Estimation
- [x] Return grams estimate and grams range per item
- [x] Return confidence fields and warnings
- [ ] Improve portion accuracy and uncertainty calibration from real usage data

#### 4.4 Multi-Photo Analysis
- [x] Accept multiple images and include suggested next shots when confidence is low
- [ ] Improve multi-angle fusion quality and occlusion handling

#### 4.5 Correction Loop
- [x] Persist correction events on save flow
- [x] Add correction analytics endpoint and dashboard visibility
- [x] Add feedback-rule auto-adjust layer from correction history
- [x] Add admin runtime toggle with persisted setting and audit trail
- [ ] Add per-user/admin policy controls for rule application

#### 4.6 Testing & Tuning
- [x] Add backend tests for correction trends endpoint
- [x] Add backend tests for runtime status endpoint
- [x] Add backend test for feedback toggle persistence and audit behavior
- [ ] Add cloud QA validation runbook execution for latest production revisions  ← **YOU — manual login on production frontend**
- [x] Add integration-level test for `/meals/analyze` endpoint path  ← `tests/test_analyze_endpoint.py` (10 tests, 2026-03-28)

**Status:** IN PROGRESS (advanced). Core AI + correction analytics shipped; remaining work is tuning, policy controls, and production QA hardening.

---

## 🤖 Checklist: What AI Can Do vs What You Need to Do

Legend: **🤖 AI** = Copilot can implement it in this session | **👤 YOU** = requires your action (device, browser, GCP console, recording)

### Milestone 1 – Flutter Capture UI
| Item | Who |
|------|-----|
| Handle camera errors gracefully (code) | 🤖 AI |
| Widget tests for CaptureScreen | 🤖 AI |
| Widget tests for ReviewScreen | 🤖 AI |
| Test camera on real Android device | 👤 YOU |
| Test navigation flow on device | 👤 YOU |
| Test on Android emulator | 👤 YOU |
| Record 30-60s demo video | 👤 YOU |

### Milestone 2 – Backend Skeleton
| Item | Who |
|------|-----|
| ✅ Integration tests for `/meals/analyze` | Done (2026-03-28) |

### Milestone 3 – Nutrition Mapping
| Item | Who |
|------|-----|
| Set up Alembic migrations (deferred) | 🤖 AI |
| `GET /meals/history` endpoint (deferred) | 🤖 AI |
| Food search endpoint (deferred) | 🤖 AI |
| End-to-end device test (capture→analyze→save→home) | 👤 YOU |

### Milestone 4 – AI Integration
| Item | Who |
|------|-----|
| Improve unknown-food handling / label normalization | 🤖 AI |
| Add per-user/admin policy controls for rule application | 🤖 AI |
| Cloud QA runbook execution (login + toggle + verify on prod) | 👤 YOU |

### Milestone 5 – Flutter Home & History (all not started)
| Item | Who |
|------|-----|
| All screen code (HomeScreen, MealListScreen, MealDetailScreen) | 🤖 AI |
| Local sqflite DB setup + cache + offline mode | 🤖 AI |
| Testing on device / multiple devices | 👤 YOU |

### Milestone 6 – Polish & Release (all not started)
| Item | Who |
|------|-----|
| UI polish, animations, error screens, onboarding flow | 🤖 AI |
| Performance profiling + image compression | 🤖 AI |
| Test on multiple devices / beta testing | 👤 YOU |
| Generate release builds (appbundle / .ipa) | 👤 YOU (needs Mac for iOS) |
| Create store screenshots + videos | 👤 YOU |
| Submit to Google Play (internal testing) | 👤 YOU |
| Submit to TestFlight | 👤 YOU (needs Mac + Apple developer account) |

### Phase P1 – Infra Audit
| Item | Who |
|------|-----|
| Confirm Cloud project ID + region from `.env` files | 🤖 AI |
| Export Firestore schema (collections + field names) | 👤 YOU (GCP/Firebase console) |

### Phase P2 – NutriLens Cloud Run
| Item | Who |
|------|-----|
| Update Flutter `api_config.dart` → Cloud Run URL | 🤖 AI |
| Deploy to Cloud Run | 👤 YOU (run deploy script) |

### Phase P4 – Google SSO
| Item | Who |
|------|-----|
| Enable Firebase Auth in GCP project (console step) | 👤 YOU |

### Phase P5 – NutriLens Admin UI
| Item | Who |
|------|-----|
| `/nutrilens/meals` meal log viewer page | 🤖 AI |
| `/nutrilens/nutrition` food DB browser/editor page | 🤖 AI |
...
> ?? **Note:** User referred to this as App Engine � it is actually **Cloud Run** (confirmed: `gcloud run deploy leave-tracker-api` in deploy script, `run.googleapis.com` API enabled).
...
## Archived Legacy Snapshot (Phase P3.6, 2026-03-08)

This old section was kept only as historical context and previously contained encoding-corrupted content.

For current implementation and next actions, use these sections instead:
- Top-of-file resume block: `RESUME HERE — 2026-03-14`
- `Cloud Integration Readiness`
- `Next After Cloud QA`
- `Current Milestone 4 Snapshot`

The current codebase has already moved beyond this P3.6 snapshot, including:
- Real correction analytics and trend endpoints
- Feedback-rule toggle persistence with audit trail
- Runtime analysis status endpoint (Gemini vs deterministic fallback)
- Dashboard integration for trends and analysis runtime visibility
